...
ggplot(diamonds_sample) + geom_point(aes(x = carat, y = price, size = price))

# Try coloring the above plot based on the diamond's price!
ggplot(diamonds_sample) + geom_point(aes(x = carat, y = price, size = price, color = price))
...
# the x-axis
ggplot(diamonds_sample) + geom_bar(aes(x = cut))

# Add an aesthetic property that will _fill_ each bar geometry based on the 
# `clarity` of the diamonds. 
# What kind of chart do you get?
ggplot(diamonds_sample) + geom_bar(aes(x = cut, fill = clarity))

# Draw a histogram (using histogram geometry) of diamond prices.
# Try mapping each bar based on clarity as well!
ggplot(diamonds_sample) + geom_histogram(aes(x = price, color = clarity))

# (For a more traditional "bell-curve", make a histogram of diamond `depth`)
ggplot(diamonds_sample) + geom_histogram(aes(x = depth, color = clarity))
...
ggplot(diamonds_sample) + geom_point(aes(x = carat, y = price) , alpha = 0.3, color = "red") + geom_smooth(aes(x = carat, y = price, color = cut), se = F)
